...
const OWNED_CHARACTERS = [
    ...
    { id: "iskorka", name: "Искорка", icon: "assets/chars/iskorka.png", eidolons: 6 },
    ...
    { id: "kastoriya", name: "Кастория", icon: "assets/chars/kastoriya.png", eidolons: 5 },
    { id: "faenon", name: "Фаенон", icon: "assets/chars/faenon.png", eidolons: 2 },
    ...
    { id: "yao_guan", name: "Яо Гуан", icon: "assets/chars/yao_guan.png", eidolons: 5 },
    ...
    { id: "midei", name: "Мидей", icon: "assets/chars/midei.png", eidolons: 0 },
    { id: "iskra", name: "Искра", icon: "assets/chars/iskra.png", eidolons: 5 },
    { id: "anaksa", name: "Анакса", icon: "assets/chars/anaksa.png", eidolons: 0 }
];
...
// --- УМНАЯ ПОДСКАЗКА SU (Фиксированная) ---
function initTooltip() {
    const tooltip = document.getElementById('smart-tooltip');
    if (!tooltip) return;

    const isHintsDisabled = localStorage.getItem('disableHints');
    if (isHintsDisabled === 'true') {
...
function closeTooltip() {
    ...
    localStorage.setItem('disableHints', 'true');
}
...
function resetHints() {
    localStorage.removeItem('disableHints');
    localStorage.removeItem('su_tooltip_closed');
    localStorage.removeItem('su_tooltip_x');
    localStorage.removeItem('su_tooltip_y');
    location.reload(); 
}
...
function renderWaves() {
    ...
    state.waves.forEach((wave, wIndex) => {
        ...
        content.innerHTML = `
            <div class="wave-header" onclick="this.parentElement.parentElement.classList.toggle('collapsed')">
                <h3>Волна ${wIndex + 1} <span style="font-size:0.8rem; color:var(--text-muted); font-weight:normal;">(${wave.enemies.length}/5)</span></h3>
                <span class="collapse-icon">▼</span>
            </div>
        `;
        
        const bodyCollapse = document.createElement('div');
        bodyCollapse.className = 'wave-body-collapse';
        bodyCollapse.style.display = 'flex';
        bodyCollapse.style.flexWrap = 'wrap';
        bodyCollapse.style.gap = '15px';
        bodyCollapse.style.width = '100%';
        ...
        wave.enemies.forEach((enemy, eIndex) => {
            ...
            eCard.innerHTML = `
                ...
                <div style="display:flex; width:100%; gap:2px; margin-top:5px; position:relative; z-index:20;">
                    <input type="number" class="enemy-level" style="flex:1; padding:5px; width:45%;" value="${enemy.level}" min="1" max="95" 
                           onchange="updateEnemyLevel(${wIndex}, ${eIndex}, this.value)" title="Уровень">
                    <input type="number" class="enemy-level" style="flex:1; padding:5px; width:45%; color:#ff6b81;" value="${enemy.max_hp || 0}" min="0" 
        ...
        `;
        bodyCollapse.appendChild(controls);
        
        content.appendChild(bodyCollapse);
...
function renderCharPickerList() {
    ...
    filtered.forEach(char => {
        ...
        card.innerHTML = `
            ...
            <button class="btn btn-primary w-100 btn-add" onclick="selectCharacter('${char.id}')" style="margin-top:auto;">Выбрать</button>
        `;
        grid.appendChild(card);
    });
}
...
// --- ЧЕЛЛЕНДЖИ И МОДИФИКАТОРЫ ---
function renderChallenges() {
    const container = document.getElementById('challenges-container');
    const globalContainer = document.getElementById('global-challenges-container');
    ...
    SPECIAL_CHALLENGES.forEach(challenge => {
        ...
        const card = document.createElement('div');
        card.className = `challenge-card ${isActive ? 'active' : ''} ${challenge.isGlobal ? 'global-challenge-card' : ''}`;
        
        let buttonHTML = '';
        if (!isUnlocked) {
            buttonHTML = `<button class="btn btn-primary w-100 btn-add" onclick="openPurchaseModal('${challenge.id}')">Купить</button>`;
        } else if (isActive) {
            buttonHTML = `
                <div style="text-align:center; color: #4cd137; font-weight:bold; margin-bottom:5px;">Активно ✅</div>
                <button class="btn btn-danger w-100 btn-add" onclick="toggleChallenge('${challenge.id}')">Убрать</button>
            `;
        } else {
            buttonHTML = `<button class="btn btn-secondary w-100 btn-add" style="border-color: var(--accent); color: var(--accent);" onclick="toggleChallenge('${challenge.id}')">Активировать</button>`;
        }

        card.innerHTML = `
            ${!isUnlocked ? `<div class="challenge-price">${challenge.price_rub} ₽</div>` : ''}
            <h4 style="${challenge.isGlobal ? 'color: var(--hsr-gold);' : ''}">${challenge.name}</h4>
            <p>${challenge.description}</p>
            <div style="margin-top: auto; width:100%; position: relative; z-index: 50;">${buttonHTML}</div>
        `;
        
        if (challenge.isGlobal && globalContainer) {
...
function setupEventListeners() {
    ...
    const headerWrapper = document.getElementById('header-controls-wrapper');
    if (mobileMenuBtn && headerWrapper) {
    ...
    // Кнопка отправки скрыта, но логика оставлена
